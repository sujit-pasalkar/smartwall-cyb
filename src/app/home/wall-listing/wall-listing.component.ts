...
export class WallListingComponent implements OnInit, AfterViewInit {
  ...
  async getAccountList(): Promise<any> {
    try {
      ...
      console.log('getAccountList api res:', items);
  ...
  createWallPopUp() {
    ...
    dialogRef.afterClosed().subscribe(
      createWallInputs => {
        console.log('createWallInputs', createWallInputs);
        if (createWallInputs != null && createWallInputs != '') {
          this.createWallApi(createWallInputs.name, createWallInputs.label);
        }
      });
  }
  ...
  createWallApi(wallName, wallLabel) {
    this.spinner.show();
    this.homeService.createWall(wallName, wallLabel, this.selectedAccountId).subscribe(successRes => {
      console.log("create wall res", successRes);
      this.spinner.hide();
  ...
  checkInputFieldEvent() {
    if (this.editWallRowindex != null) {
      console.log('clickedd', this.tempArrayForOriginalData[this.editWallRowindex]);
      let obj = {
  ...
  clickWallName(index) {
    if (this.editWallRowindex == null) {
      ...
      console.log('wallNmw index', this.editWallRowindex);
      this.tempArrayForOriginalData.forEach(element => {
        element.editWallName = false;
        element.editWallLabel = false;
      });
  ...
  clickWallLabel(index) {
    if (this.editWallRowindex == null) {
      console.log('walllabel index', index);
      this.editWallRowindex = index;
  ...
  showWallTable(accountId, accountName) {
    if (!accountId) {
      console.log('client argument is null');
    }
    ...
    this.homeService.getSelectedAccountsWall(accountId).subscribe(wallData => {
      ...
      console.log('wallData', this.tempArrayForOriginalData);
  ...
  deleteWallStatus(wallId, wallName) {
    ...
    this.homeService.deleteWallStatus(wallId).subscribe(res => {
      console.log('delete status res', res);
      this.spinner.hide();
  ...
  editWall(wallName, wallId) {
    console.log('wallName', wallName, wallId);
    let passObject = {
      'wallId': wallId,
      'wallName': wallName,
      'wallLabel': ''
    ...
    //   this.commonService.setLabel(sname);
    //   localStorage.setItem('wallId', sId);
    //   localStorage.setItem('wallName', sname);
